...
var DefaultProfileImagePath = "/static/img/default_profile_image.png"
...
func (h *handler) UpdateImage(c *gin.Context) {
	...
	if user.ImageURL != DefaultProfileImagePath {
		err = h.userService.DeleteImageFromAWSBucket(awsSession, user.ImageURL, MyBucket, objectsLocationURL, h.infoLog)
		if err != nil {
			h.errors.ServerError(c, err)
			return
		}
